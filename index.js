...
function renderTable(arr) {
  return arr
    .map((el, i, a) => {
      ...
      <button type="button" class="archiv"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#7f8383" width="32" height="32" viewBox="0 0 32 32">
      <title>box-add</title>
      ...
      <button type="button" class="del"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#7f8383" width="32" height="32" viewBox="0 0 32 32">
      <title>bin</title>
...
function removeNote(e) {
  if (e.target.parentNode.classList.contains("del")) {
    return deleteNote(e.target.parentNode.parentNode.parentNode.parentNode.id);
  }
}
...
function openModalChange(e) {
  if (e.target.parentNode.classList.contains("edit")) {
    ...
    getNoteId(e.target.parentNode.parentNode.parentNode.parentNode.id).then(
      (data) => {
...
function submitChangeForm(e) {
  ...
  const newTask = {
    ...
    date: (refs.dateControl1.value = refs.dateControl1.value + " " + refs.dateControl2.value),
    isActive: true,
  };
...
function archivModal(e) {
  if (e.target.parentNode.classList.contains("archiv")) {
...
function renderTable3(arr) {
  if (arr.length > 0) {
    return arr
      ...
      <td>${el.todo}</td>
      <td>${el.date}</td>
      <td><button type="button" class="unarchiv">ARCH</button></td>
      </tr>`;
      })
      .join("");
  }
